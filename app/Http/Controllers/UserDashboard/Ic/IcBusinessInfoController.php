...
class IcBusinessInfoController extends Controller
{
    public function index(){
        $data['categories'] = Category::where('status',1)->get();
        $data['businessHour'] = BusinessHour::where('provider_id', Auth::user()->id)->first();
        $data['businessHourUpdate'] = BusinessHourUpdate::where('provider_id', Auth::user()->id)->where('saloon_id' , null)->first();
        $data['socialCheck'] = IcSocialMedia::where('ic_id',Auth::user()->id)->count();
        $data['socialmedia'] = IcSocialMedia::where('ic_id',Auth::user()->id)->first();
        $data['whCheck'] = BusinessHourUpdate::where('provider_id', Auth::user()->id)->where('saloon_id' , null)->count();
        $data['portfolios'] = IcPortfolio::where('ic_id',Auth::user()->id)->get();
    ...
    public function addsocial(Request $request)
    {
        ...
        $data = new IcSocialMedia();
        $data->ic_id   = Auth::user()->id;
    ...
    public function addportfolio(Request $request)
    {
        ...
        $data = new IcPortfolio();
        $data->ic_id   = Auth::user()->id;
        $portfolio_image = $request->file('portfolio_image');
        if ($portfolio_image) {

            $imageName = time() . '_' . uniqid() . '.' . $portfolio_image->getClientOriginalExtension();
            $portfolio_image->move(public_path('uploaded/provider/ic/portfolio'), $imageName);
            $data->portfolio_image = '/uploaded/provider/ic/portfolio/' . $imageName;
        }
        $data->save();
